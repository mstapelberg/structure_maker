...
class CustomStructure:
    def __init__(self, lattice, species, coords, coords_are_cartesian=False, to_unit_cell=False):
        ...
        if to_unit_cell:
            self.apply_to_unit_cell()

    def apply_to_unit_cell(self):
        """Ensure coordinates are within the unit cell."""
    ...
    def as_dict(self):
        ...
        return {
            ...
            "to_unit_cell": self.to_unit_cell
        }

    @classmethod
    def from_dict(cls, d):
        """Create a CustomStructure from a dictionary."""
        return cls(
            lattice=d["lattice"],
            species=d["species"],
            coords=d["coords"],
            coords_are_cartesian=d.get("coords_are_cartesian", False),
            to_unit_cell=d.get("to_unit_cell", False)
        )
...
class VASPDataParser:
    ...
    def get_structures(self, filename="XDATCAR", lines=None):
        """Creates CustomStructure instances from the XDATCAR file."""
        trigger_indices, lines = self._get_trigger(
            lines=lines,
            filename=filename,
        ...
        lattice_vector = [lines[trigger_indices[0] - 5 + i].split() for i in range(3)]
        lattice_vector = np.array(lattice_vector).astype(float).tolist()
        species_lst = [species[i] for i in range(len(species)) for j in range(int(number[i]))]
        ...
        for i in range(len(trigger_indices)):
            positions = [lines[trigger_indices[i] + 1 + j].split() for j in range(len(species_lst))]
            positions = np.array(positions).astype(float).tolist()
    ...
    def _get_structures(self, filename="XDATCAR", lines=None):
